...
parser.add_argument("-z","--zero", action="store_true" ,help="0 labels, default 1")
...
ts_image = experiment["files_path"]["ts_image"] if args.msdl else experiment["files_path"]["ts_image"]
...
split = "control" if labeled == 0 else "et"
experiment["split"] = split
main_dir = data_dir+'/'+split+'/'+ algorithm
create_dir(main_dir)
msdl_dir = data_dir+'/'+split+'/'+'msdl'
create_dir(msdl_dir)
...
if args.msdl:
  ...
  plotting.plot_prob_atlas(masker.maps_img, view_type='filled_contours',
                           title=title,
                           output_file=op.join(msdl_dir,"_func_map_ica_"+str(masker_n_regions_extracted)+".png")
                           )
  ...
  mask_mean_correlations = np.mean(masker_correlations, axis=0).reshape(len(labels),
                                                            len(labels))
  experiment["files_path"]["msdl_ts_file"]="msdl/masker_extracted_ts.csv"
...
# In[]
  ...
  #plt.title(title)
  fig.savefig(os.path.join(msdl_dir,"mask_mean_correlation_matrix.png"))

  if(args.plot_components):      
    plot(atlas_filename,filepath=op.join(msdl_dir,"_resting_state_ica_"),num_components=len(labels))
  # Plot resulting connectcomes
  
  if(args.plot_connectcome):
      plot_connectcome(masker_extracted_img, mask_mean_correlations, "", op.join(msdl_dir,'_plot_connectcome_all_mean.png'))
      # Now, we plot (right side) same network after region extraction to   that connected regions are nicely seperated.
      plot_extracted(atlas.maps,masker_extracted_img, len(masker.labels_),masker.labels_,os.path.join(msdl_dir,"_resting_state_ica_regions"))
