...
from nipype.interfaces.fsl import MCFLIRT, BET, SUSAN, SliceTimer, TemporalFilter, ImageStats, Threshold, \
    FilterRegressor
...
def experiment(config):
    ...
    # logging
    # logging.getLogger("preproc").setLevel(config["log_level"])
    # logging.basicConfig(filename=config["files_path"]["preproc"]["log"], filemode='w',
    #                     format="%(asctime)s - %(levelname)s - %(message)s")

    logger = logging.init(config["files_path"]["preproc"]["log"], config["log_level"], config["log_level"])

    # set working dirs
    base_dir, data_dir, experiment_dir, output_dir, subject_list = get_dirs(config)

    # session id list
    session_list = [1]  # TODO allow more than one session

    # smoothing filters threshold
    ...
    # Band pass filter
    bandpass_filter = Node(TemporalFilter(highpass_sigma=args.highpass, lowpass_sigma=args.lowpass),
    ...
    # fetch input
    infosource = Node(
        IdentityInterface(fields=['subject_id', 'session_id', 'Brain_template', 'Template', 'Template_3mm']),
        name="infosource")
    ...
    preproc.inputs.infosource.Brain_template = opj(base_dir,
                                                   experiment["files_path"]["preproc"]["register"]["brain_template"])
    preproc.inputs.infosource.Template_3mm = opj(base_dir,
                                                 experiment["files_path"]["preproc"]["register"]["template_3mm"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--config", type=str, help="Configuration file path, default file is config_old.json",
                        nargs='?', default="conf/config_test.json")
    parser.add_argument("-m", "--move_plot", action="store_true",
                        help="MCFLIRT: Plot translation and rotations movement and save .par files")
    parser.add_argument("-f", "--fwhm", type=str, help="Smooth filter's threshold list. [5] by default", nargs='?',
                        default="[5]")
    parser.add_argument("-b", "--brightness_threshold", type=float,
                        help="Smooth filter brightness' threshold. 1000.0 by default", nargs='?', default=None)
    parser.add_argument("-p", "--parallelism", type=int,
                        help="Multiproc parallelism configuration, default no parallelism", nargs='?', default=2)
    ...
    config = experiment_config(args.config)["experiment"]
